...
class TestRNN(unittest.TestCase):
    def test_rnn_cell1(self):
        ...
        hidden_dim = 128
        batch_size = 1
        self.perform_rnn_cell_test(input_dim, hidden_dim, batch_size)
        print("Passed")
    ...
    def perform_rnn_cell_test(self, input_dim, hidden_dim, batch_size):
        ...
        self.assertTrue(torch.allclose(my_out, out, atol=1e-5), "Outputs are not close")
    ...
    def test_rnn1(self):
        ...
        batch_size = 2
        seq_len = 5
        self.perform_rnn_test(input_size, hidden_size, batch_size, seq_len)
        print("Passed")
    ...
    def perform_rnn_test(self, input_size, hidden_size, batch_size, seq_len):
        num_layers = 1
        batch_first = True
        nonlinearity = "relu"
    ...
    def test_attention1(self):
        ...
        embed_dim = 256
        kdim = embed_dim  # Pytorch doesn't like giving us the q bias if the dims are not equal
        vdim = embed_dim
        batch_size = 1
        seq_len = 5
        self.perform_attention_test(embed_dim, kdim, vdim, batch_size, seq_len)
        print("Passed")
    ...
    def perform_attention_test(self, embed_dim, kdim, vdim, batch_size, seq_len):
        num_heads = 1
        dropout = 0.0
        bias = True
